...
dat = readRDS("../data/from_imdb_so.rds")
datw = readRDS("../data/from_imdb_sow.rds")
...
```{r basics}
ggplot(dat) + aes(imdb) + geom_histogram() + scale_x_continuous(breaks = (0:5)*2) + xlim(c(3,10))
ggplot(data.frame(table(dat$director))) + aes(x=Var1,y=Freq) + geom_col() 
print(table(dat$director, dat$show))
```

```{r bivariate}
ggplot(dat) + aes(x=show_f, y=imdb) + geom_boxplot()
for(sh in shows){
  print(
    ggplot(dat[show==sh]) + aes(x=season_f, y=imdb) + geom_boxplot()
  )
}
```
...
```{r cast}
charac_power= list()
charac_power$TNG = sapply(maincast$TNG, function(z) cor(dat[show=="TNG"]$imdb, dat[show=="TNG"][[paste0("cast_",z)]]) ) %>% data.table(charac = names(.), corr = .)
charac_power$DS9 = sapply(maincast$DS9, function(z) cor(dat[show=="DS9"]$imdb, dat[show=="DS9"][[paste0("cast_",z)]]) ) %>% data.table(charac = names(.), corr = .)
charac_power$VOY = sapply(maincast$VOY, function(z) cor(dat[show=="VOY"]$imdb, dat[show=="VOY"][[paste0("cast_",z)]]) ) %>% data.table(charac = names(.), corr = .)
charac_power$ENT = sapply(maincast$ENT, function(z) cor(dat[show=="ENT"]$imdb, dat[show=="ENT"][[paste0("cast_",z)]]) ) %>% data.table(charac = names(.), corr = .)

for(cp in charac_power){
  print(
    ggplot(cp) + aes(x=charac, y=corr) + geom_col()
  )
}
```
...
f3 = paste("imdb ~",f3, " + partofseason + s(1|season_f) + (1|director_f)") %>% as.formula
...
for(sh in names(maincast)){
  ...
  print(
    ggplot() + aes(x=charac, y=Estimate) + geom_point(data=sm3sdat) + 
      geom_errorbar(aes(x=charac,ymin = lower, ymax = upper,y=NULL), data=cimsdat) +
      ggtitle(sh) 
  )
}
```
